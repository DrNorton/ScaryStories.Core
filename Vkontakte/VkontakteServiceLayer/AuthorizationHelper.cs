...
namespace VkontakteServiceLayer
{
    public class AuthorizationHelper
    {
        ...
        public static string GetAuthorizationUrl(string applicationId)
        {
            ...
            const string url = "https://oauth.vk.com/authorize?client_id={0}&scope=notify,friends,photos,audio,video,docs,notes,pages,offers,questions,wall,messages&redirect_uri={1}&display=touch&response_type=token";
        ...
        private static AuthorizationResult GetAuthResult(Dictionary<string, string> tokenArray)
        {
            ...
            else
            {
                ...
                authorizationResult.Context = new AuthorizationContext()
                {
                    ...
                    ExpiresInSeconds = int.Parse(tokenArray["expires_in"]),
                    AuthroizationTime = DateTime.Now,
