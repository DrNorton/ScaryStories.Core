...
using Message = VkontakteInfrastructure.Model.Message;
using Photo = VkontakteInfrastructure.Model.Photo;
using PhotoAlbum = VkontakteInfrastructure.Model.PhotoAlbum;
...
namespace VkontakteCore
{
    ...
    public class VkontakteApi : IVkontakteApi
    {
        ...
        public void WallPost(string uid, string textmessage, Action sendMessageCompleteAction, Action<Error> sendMessageErrorAction)
        {
            var serviceLayer = new ServiceLayer();
            serviceLayer.WallPost(Context, uid, textmessage, sendMessageCompleteAction, sendMessageErrorAction);
        }

        public bool Authorized() {
            var context = entityDataStorage.LoadEntity<AuthorizationContext>();
            bool entityNotNull=context!=null;
            bool isntExpired = false;
            if (context != null) {
               isntExpired= (context.AuthroizationTime.AddSeconds(context.ExpiresInSeconds) > DateTime.Now);  
            }
            return entityNotNull & isntExpired;
        }
