...
namespace OpenNETCF.ORM
{
        ...
        where TEntityInfo : EntityInfo, new()
    {
        ...
        protected abstract IEnumerable<object> Select(Type objectType, IEnumerable<FilterCondition> filters, int fetchCount, int firstRowOffset, bool fillReferences,int limit);
        ...
        public abstract override IEnumerable<object> Fetch(int fetchCount, string sortField, FieldSearchOrder sortOrder, bool fillReferences, Type objectType);
        ...
        {
            return (T)Select(typeof(T), null, primaryKey, -1, -1, true,0).FirstOrDefault();
        }

        public override T Select<T>(object primaryKey, bool fillReferences)
        {
            return (T)Select(typeof(T), null, primaryKey, -1, -1, fillReferences,0).FirstOrDefault();
        }
        ...
        public  IEnumerable<T> SelectRandom<T>()
        {
            var type = typeof(T);
            var items = Select(type, null, null, -1, 0,10);
            return items.Cast<T>();
        }
        ...
        {
            var type = typeof(T);
            var items = Select(type, null, null, -1, 0, fillReferences,0);
            return items.Cast<T>();
        }

        public override IEnumerable<T> Select<T>(bool fillReferences,int limit)
        {
            var type = typeof(T);
            var items = Select(type, null, null, -1, 0, fillReferences,limit);
            return items.Cast<T>();
        }
        ...
        {
            var items = Select(entityType, null, null, -1, 0, fillReferences,0);
            return items;
        }
        ...
        {
            var type = typeof(T);
            var items = Select(type, searchFieldName, matchValue, -1, 0, fillReferences,0);
            return items.Cast<T>();
        }
        ...
        public override IEnumerable<T> Select<T>(IEnumerable<FilterCondition> filters, bool fillReferences)
        {
            ...
            return Select(objectType, filters, -1, 0, fillReferences,0).Cast<T>();
        }
        ...
        {
            return Select(objectType, searchFieldName, matchValue, fetchCount, firstRowOffset, true,0);
        }

        private IEnumerable<object> Select(Type objectType, string searchFieldName, object matchValue, int fetchCount, int firstRowOffset,int limit)
        {
            return Select(objectType, searchFieldName, matchValue, fetchCount, firstRowOffset, true, limit);
        }

        protected virtual IEnumerable<object> Select(Type objectType, string searchFieldName, object matchValue, int fetchCount, int firstRowOffset, bool fillReferences,int limit)
        {
        ...
        {
            tableDirect = false;
            return BuildFilterCommand<TCommand, TParameter>(entityName, filters,0);
        }

        protected virtual TCommand GetSelectCommand<TCommand, TParameter>(string entityName, IEnumerable<FilterCondition> filters, out bool tableDirect,int limit)
            where TCommand : DbCommand, new()
            where TParameter : IDataParameter, new()
        {
            tableDirect = false;
            return BuildFilterCommand<TCommand, TParameter>(entityName, filters,limit);
        }

        protected virtual TCommand GetSelectCommand<TCommand, TParameter>(string entityName,FieldSearchOrder sortOrder,string sortField,out bool tableDirect,int fetchCount)
            where TCommand : DbCommand, new()
            where TParameter : IDataParameter, new()
        {
            tableDirect = false;
            ...
            sb = new StringBuilder(string.Format("SELECT * FROM [{0}] ORDER BY VIEWTIME DESC LIMIT {1}", entityName, fetchCount));
        ...
        protected TCommand BuildFilterCommand<TCommand, TParameter>(string entityName, IEnumerable<FilterCondition> filters,int limit)
            where TCommand : DbCommand, new()
            where TParameter : IDataParameter, new()
        {
            return BuildFilterCommand<TCommand, TParameter>(entityName, filters, false,limit);
        }

        protected TCommand BuildFilterCommand<TCommand, TParameter>(string entityName, IEnumerable<FilterCondition> filters, bool isCount,int limit)
            where TCommand : DbCommand, new()
            where TParameter : IDataParameter, new()
        {
            var command = new TCommand();
            ...
            StringBuilder sb;
            if (limit != 0) {
                sb = new StringBuilder(string.Format("SELECT * FROM [{0}] ORDER BY RANDOM() LIMIT {1}", entityName,limit));
            }
            else {
                if (isCount) {
        ...
        protected void DoInsertReferences(object item, string entityName, KeyScheme keyScheme)
        {
            ...
            foreach (var reference in Entities[entityName].References)
            {
                if (reference.ReferenceType == ReferenceType.ManyToOne) // N:1
                {
                    ...
                    var existing = Select(reference.ReferenceEntityType, null, refPK, -1, -1, true,0).FirstOrDefault();
            ...
            foreach (var reference in Entities[entityName].References)
            {
                ...
                if (!referenceItems.ContainsKey(reference))
                {
                    ...
                    else
                    {
                        ...
                        refData = Select(reference.ReferenceEntityType, reference.ReferenceField, keyValue, -1, 0, false,0);
                    }
...
//                    reference.PropertyInfo.SetValue(instance, Convert.ChangeType(carr, reference.PropertyInfo.PropertyType), null);
        ...
        {
            var type = typeof(T);
            var items = Select(type, null, null, fetchCount, 0, false,0);
            return items.Cast<T>();
        }
        ...
        {
            var type = typeof(T);
            var items = Select(type, null, null, fetchCount, firstRowOffset, false,0);
            return items.Cast<T>();
        }
